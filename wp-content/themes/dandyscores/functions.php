 ...
 */
function dandyscores_content_image_sizes_attr( $sizes, $size ) {
	$width = $size[0];
...
add_filter( 'wp_calculate_image_sizes', 'dandyscores_content_image_sizes_attr', 10, 2 );
 ...
 */
function dandyscores_header_image_tag( $html, $header, $attr ) {
	if ( isset( $attr['sizes'] ) ) {
		$html = str_replace( $attr['sizes'], '100vw', $html );
	}
	return $html;
}
add_filter( 'get_header_image_tag', 'dandyscores_header_image_tag', 10, 3 );
 ...
 */
function dandyscores_post_thumbnail_sizes_attr( $attr, $attachment, $size ) {
	if ( !is_singular() ) {
		if ( is_active_sidebar ( 'sidebar-1') ) {
			$attr['sizes'] = '(max-width: 900px) 90vw, 800px';
		}else {
		$attr['sizes'] = '(max-wdith: 1000px) 90vw, 1000px';
		}
	}

	return $attr;
}
add_filter( 'wp_get_attachment_image_attributes', 'dandyscores_post_thumbnail_sizes_attr', 10, 3 );
